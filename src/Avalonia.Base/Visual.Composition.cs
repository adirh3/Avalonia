...
public partial class Visual
{
    ...
    private Func<Vector3> _getCenterPoint = () => Vector3.Zero;
    private Func<Vector3> _getOffset = () => Vector3.Zero;
    ...
    internal CompositionVisual AttachToCompositor(Compositor compositor)
    {
        if (CompositionVisual == null || CompositionVisual.Compositor != compositor)
        {
            ...
            UpdateCompositionPropertyGetters(CompositionVisual);
        }

        return CompositionVisual;
    }

    private void UpdateCompositionPropertyGetters(CompositionDrawListVisual compositionVisual)
    {
        if (compositionVisual.Server is ServerCompositionDrawListVisual serverCompositionDrawListVisual)
        {
        ...
        {
            ResetCompositionPropertyGetters();
        }
    }

    private void ResetCompositionPropertyGetters()
    {
        _getScale = () => Vector3.One;
        _getOpacity = () => 1f;
        _getCenterPoint = () => Vector3.Zero;
        _getOffset = () => Vector3.Zero;
    }
