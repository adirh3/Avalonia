...
internal interface IWinUiCompositionWindowInfo
{
...
internal class WinUiCompositedWindow : IDisposable
{
    ...
    public IWinUiCompositionWindowInfo CompositionInfo { get; }
    ...
    private double _scaling = double.NaN;
        ...
        WinUiCompositionShared shared)
    {
        ...
        _compositionRoundedRectangleGeometry = CreateRoundedRectangleGeometry(shared.Compositor);
    ...
    public void SetBlur(BlurEffect blurEffect)
    {
        ...
        lock (_shared.SyncRoot)
        {
            ...
            if (_currentVisual != null)
            {
                ApplyRoundedClip(_currentVisual);
                ...
                ResizeIfNeeded(
                    WindowInfo.Size,
                    WindowInfo.Scaling,
                    CompositionInfo.WindowState,
                    CompositionInfo.CompositionPadding,
                    CompositionInfo.ScaleTransform,
                    CompositionInfo.CenterPoint,
                    CompositionInfo.Opacity,
                    CompositionInfo.Offset,
                    CompositionInfo.CompositionCornerRadius);
            }
        }
    }

    private static ICompositionRoundedRectangleGeometry CreateRoundedRectangleGeometry(ICompositor compositor)
    {
        using var compositor5 = compositor.QueryInterface<ICompositor5>();
        return compositor5.CreateRoundedRectangleGeometry();
    }

    private void ApplyRoundedClip(IVisual visual)
    {
        ...
        using var clip = geometricClipWithGeometry.QueryInterface<ICompositionClip>();
        visual.SetClip(clip);
    }
    ...
    private ICompositionBrush? CreateMicaBackdropBrush(float color, float opacity)
    {
        ...
        using var backgroundParameter = new EffectSourceParameter("Background", backDropParameterFactory);
        using var foregroundParameter = new EffectSourceParameter("Foreground", backDropParameterFactory);

        using var luminosityBlendEffect =
            new BlendEffect(23, backgroundParameter.Source, foregroundParameter.Source);
        ...
        luminosityBlendEffectBrush.SetSourceParameter(backgroundParameter.Handle, micaBackdropBrush);
        luminosityBlendEffectBrush.SetSourceParameter(foregroundParameter.Handle, luminosityOpacityEffectBrush);


        using var backgroundParameter1 = new EffectSourceParameter("Background", backDropParameterFactory);
        using var foregroundParameter1 = new EffectSourceParameter("Foreground", backDropParameterFactory);

        using var colorBlendEffect =
            new BlendEffect(22, backgroundParameter1.Source, foregroundParameter1.Source);
        ...
        colorBlendEffectBrush.SetSourceParameter(backgroundParameter1.Handle, luminosityBlendEffectBrush1);
        colorBlendEffectBrush.SetSourceParameter(foregroundParameter1.Handle, tintOpacityEffectBrush);
    ...
    private ICompositionBrush? CreateAcrylicBlurBackdropBrush()
    {
        ...
        using var saturationEffectFactory = _shared.Compositor.CreateEffectFactory(saturateEffect);
        using var saturationEffectBrush = saturationEffectFactory.CreateBrush();
        saturationEffectBrush.SetSourceParameter(backdropString.Handle, backdropBrush);
        return saturationEffectBrush.QueryInterface<ICompositionBrush>();
    }
    ...
    private sealed class EffectSourceParameter : IDisposable
    {
        private readonly HStringInterop _name;
        private readonly ICompositionEffectSourceParameter _parameter;

        public EffectSourceParameter(string name, ICompositionEffectSourceParameterFactory factory)
        {
            _name = new HStringInterop(name);
            _parameter = factory.Create(_name.Handle);
            Source = _parameter.QueryInterface<IGraphicsEffectSource>();
        }

        public IntPtr Handle => _name.Handle;
        public IGraphicsEffectSource Source { get; }
